...
export class InspectionDataSynchronizerProvider extends BaseDataSynchronizerProvider<Batch[]> {
  ...
  private downloadBuildingsData(idBuildings: string[]): Promise<boolean>[]{
    ...
    for(let idBuilding of idBuildings){
      ...
      promises.push(this.downloadDataAndSavePicturesByParent('inspection/building/' + idBuilding + '/anomaly/pictures'));
    }
    return promises;
  }
  ...
  private downloadDataAndSavePicturesByParent(url: string): Promise<boolean>{
    return new Promise((resolve) => {
        ...
        .subscribe(
          async (data: InspectionBuildingAnomalyPictures[]) => {
            data.forEach(async(anomaly) => {
              await this.saveData<InspectionPictureForWeb[]>(anomaly.pictures, anomaly.id, 'building_anomaly_pictures_');
            })
            resolve(true);
          },
          async () => resolve(false)
